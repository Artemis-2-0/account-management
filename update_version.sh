...
# Extract the version below <artifactId>account-management</artifactId>
CURRENT_VERSION=$(awk '
...
if [[ -z "$CURRENT_VERSION" ]]; then
    echo "Error: Unable to find <version> tag for <artifactId>account-management</artifactId> in pom.xml"
    exit 1
fi
...
# Update only the version below <artifactId>account-management</artifactId>
