drop table if exists `acctmgmtdb`.`user_role`;
drop table if exists `acctmgmtdb`.`role_authority`;
drop table if exists `acctmgmtdb`.`security_user`;
drop table if exists `acctmgmtdb`.`role`;
drop table if exists `acctmgmtdb`.`authority`;
CREATE TABLE IF NOT EXISTS `acctmgmtdb`.`security_user` (
...
CREATE TABLE IF NOT EXISTS `acctmgmtdb`.`role` (
...
CREATE TABLE IF NOT EXISTS `acctmgmtdb`.`authority` (
...
CREATE TABLE IF NOT EXISTS `acctmgmtdb`.`user_role` (
    ...
    CONSTRAINT `role_id`
    FOREIGN KEY (`role_id`)
    REFERENCES `acctmgmtdb`.`role` (`role_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
    ENGINE = InnoDB
    COMMENT = 'The mapping table, to map the users with respective roles.';

CREATE TABLE IF NOT EXISTS `acctmgmtdb`.`role_authority` (
    ...
    REFERENCES `acctmgmtdb`.`authority` (`authority_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
    ENGINE = InnoDB
